...
void interrupt newI70h(...) { 
	...
	outp(0x20, 0x20);				// Посылаем контроллеру прерываний (master)
									// сигнал EOI (end of interrupt)
	outp(0xA0, 0x20);				// Посылаем второму контроллеру прерываний(slave)
									// сигнал EOI (end of interrupt)
}
...
void delayTime() {
	...
	value = inp(0xA1) & 0xFE;		// 0xFE = 11111110, бит 0 в 0,
									// чтобы разрешить прерывания от ЧРВ
	// Включение периодического прерывания
	outp(0xA1, value);
	outp(0x70, 0x0B);				// Выбираем регистр B
	value = inp(0x71) | 0x40;		// Читаем содержимое регистра B, 6-й бит регистра B устанавливаем в 1
	outp(0x70, 0x0B);				// Выбираем регистр B
	outp(0x71, value);
	milsec = 0;
